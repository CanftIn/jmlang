#ifndef JMLANG_IR_IR_OPERATOR_H
#define JMLANG_IR_IR_OPERATOR_H
...
/// be converted to Jmlang Expr or to std::string.  The problem
...
/// Jmlang's vectorizable transcendentals.
Expr jmlang_log(Expr a);
Expr jmlang_exp(Expr a);
...
/// Cast an expression to the jmlang type corresponding to the C++ type T.
template <typename T>
...
inline Expr atan2(Expr y, Expr x) {
  ...
  if (y.type() == Float(64)) {
    x = cast<double>(x);
    return internal::Call::make(Float(64), "atan2_f64", vec(y, x),
                                internal::Call::Extern);
  } else {
    y = cast<float>(y);
    x = cast<float>(x);
    return internal::Call::make(Float(32), "atan2_f32", vec(y, x),
                                internal::Call::Extern);
  }
}
...
inline Expr exp(Expr x) {
  ...
  if (x.type() == Float(64)) {
    ...
    // internal::Call::Extern);
    return internal::jmlang_exp(cast<float>(x));
  }
}
...
inline Expr log(Expr x) {
  ...
  if (x.type() == Float(64)) {
    ...
    // internal::Call::Extern);
    return internal::jmlang_log(cast<float>(x));
  }
}
...
inline Expr pow(Expr x, Expr y) {
  ...
  if (x.type() == Float(64)) {
    ...
    x = cast<float>(x);
    y = cast<float>(y);
    return internal::jmlang_exp(internal::jmlang_log(x) * y);
  }
}
...
inline Expr reinterpret(Type t, Expr e) {
  ...
  return internal::Call::make(t, internal::Call::reinterpret, vec(e),
...
inline Expr operator&(Expr x, Expr y) {
  ...
  return internal::Call::make(x.type(), internal::Call::bitwise_and, vec(x, y),
                              internal::Call::Intrinsic);
}
...
inline Expr operator|(Expr x, Expr y) {
  ...
  return internal::Call::make(x.type(), internal::Call::bitwise_or, vec(x, y),
                              internal::Call::Intrinsic);
}
...
inline Expr operator^(Expr x, Expr y) {
  ...
  return internal::Call::make(x.type(), internal::Call::bitwise_xor, vec(x, y),
                              internal::Call::Intrinsic);
}
...
inline Expr operator~(Expr x) {
  ...
  return internal::Call::make(x.type(), internal::Call::bitwise_not, vec(x),
                              internal::Call::Intrinsic);
}

/// Shift the bits of an integer value left. This is actually less
/// efficient than multiplying by 2^n, because Jmlang's optimization
...
inline Expr operator<<(Expr x, Expr y) {
  ...
  return internal::Call::make(x.type(), internal::Call::shift_left, vec(x, y),
                              internal::Call::Intrinsic);
}
...
/// two. Jmlang's definition of division (always round to negative
/// infinity) means that all divisions by powers of two get compiled to
/// bit-shifting, and Jmlang's optimization routines understand
...
inline Expr operator>>(Expr x, Expr y) {
  ...
  internal::match_types(x, y);
  return internal::Call::make(x.type(), internal::Call::shift_right, vec(x, y),
                              internal::Call::Intrinsic);
}
...
/// Some examples:
/// \code
///     // Since Jmlang does not have direct type delcarations, casts
...
///       cast<uint8_t>(cast<uint8_t>(x)/// (1.0f - cast<uint8_t>(w) /
///       255.0f) +
///                     cast<uint8_t>(y)/// cast<uint8_t>(w) / 255.0f + .5f)
///     // Note addition in Jmlang promoted uint8_t + int8_t to int16_t
///     already,
///     // the outer cast is added for clarity.
///     lerp(cast<uint8_t>(x), cast<int8_t>(y), cast<uint8_t>(w)) ->
///       cast<int16_t>(cast<uint8_t>(x)/// (1.0f - cast<uint8_t>(w) /
///       255.0f) +
  ...
  if (!zero_val.type().is_float()) {
    ...
    if (const_weight != NULL) {
      assert(*const_weight >= 0.0f && *const_weight <= 1.0f &&
             "floating-point weight for lerp with integer arguments must be "
