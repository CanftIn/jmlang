#ifndef JMLANG_IR_FUNCTION_H
#define JMLANG_IR_FUNCTION_H
...
struct FunctionContents {
  ...
  FunctionContents()
      : trace_loads(false), trace_stores(false), trace_realizations(false) {}
};

/// A reference-counted handle to internal representation of
...
class Function {
 ...
 public:
  ...
  void define_reduction(const std::vector<Expr>& args,
                        std::vector<Expr> values);
  ...
  /// Get a handle to the schedule for the purpose of modifying it.
  ...
  /// Get a mutable handle to the schedule for the reduction stage.
  Schedule& reduction_schedule() { return contents.ptr->reduction_schedule; }
