#ifndef JMLANG_LANG_IMAGE_H
#define JMLANG_LANG_IMAGE_H
...
class Image {
 private:
  ...
  void prepare_for_direct_pixel_access() {
    ...
    if (buffer.defined()) {
      ...
      origin -= (buffer.min(0) * stride_0 + buffer.min(1) * stride_1 +
                 buffer.min(2) * stride_2 + buffer.min(3) * stride_3);
 ...
 public:
  ...
  /// pixels.
  Image(const buffer_t* b, const std::string& name = "")
      : buffer(type_of<T>(), b, name) {
    prepare_for_direct_pixel_access();
  }
  ...
  /// existing image.
  void copy_to_host() { buffer.copy_to_host(); }
  ...
  void set_host_dirty(bool dirty = true) { buffer.set_host_dirty(dirty); }

  /// Check if this image handle points to actual data.
  bool defined() const { return buffer.defined(); }

  /// Get the dimensionality of the data. Typically two for grayscale images,
  /// and three for color images.
  ...
  int extent(int dim) const {
    ...
    assert(dim >= 0 && dim < dims &&
           "dimension out of bounds in call to Image::extent");
  ...
  int min(int dim) const {
    assert(defined());
    assert(dim >= 0 && dim < dims &&
           "dimension out of bounds in call to Image::min");
  ...
  int stride(int dim) const {
    assert(defined());
    assert(dim >= 0 && dim < dims &&
           "dimension out of bounds in call to Image::stride");
  ...
  T operator()(int x) const { return origin[x]; }
  ...
  /// element at position x.
  T& operator()(int x) { return origin[x * stride_0]; }

  /// Assuming this image is two-dimensional, get a reference to the
  /// element at position (x, y).
  T& operator()(int x, int y) { return origin[x * stride_0 + y * stride_1]; }
