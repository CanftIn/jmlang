...
class IREquals : public IRVisitor {
 public:
  ...
  void visit(const IntImm* op) {
    if (result || expr.same_as(op) || compare_node_types(expr, op))
      return;
  ...
  int compare_types(Type a, Type b) {
    if (a.type_kind < b.type_kind) {
      result = -1;
    } else if (a.type_kind > b.type_kind) {
      result = 1;
  ...
  void visit(const Load* op) {
    ...
    if (compare_names(op->name, e->name))
      return;
  ...
  void visit(const Call* op) {
    ...
    if (compare_names(e->name, op->name))
      return;
  ...
  void visit(const LetStmt* op) {
    if (result || stmt.same_as(op) || compare_node_types(stmt, op))
      return;

    const LetStmt* s = stmt.as<LetStmt>();

    if (compare_names(s->name, op->name))
      return;
  ...
  void visit(const AssertStmt* op) {
    ...
    if (compare_names(s->message, op->message))
      return;
...
bool equal(Expr a, Expr b) { return deep_compare(a, b) == 0; }

bool equal(Stmt a, Stmt b) { return deep_compare(a, b) == 0; }
