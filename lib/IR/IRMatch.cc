#include "jmlang/IR/IRMatch.h"

#include <iostream>

#include "jmlang/IR/IREquality.h"
...
void expr_match_test() {
  ...
  assert(expr_match(w, 3, matches) && equal(matches[0], 3));

  assert(expr_match(w + 3, (y * 2) + 3, matches) && equal(matches[0], y * 2));

  assert(expr_match(fw * 17 + cast<float>(w + cast<int>(fw)),
                    (81.0f * fy) * 17 + cast<float>(x / 2 + cast<int>(4.5f)),
                    matches) &&
         equal(matches[0], 81.0f * fy) && equal(matches[1], x / 2) &&
         equal(matches[2], 4.5f));

  assert(!expr_match(fw + 17, fx + 18, matches) && matches.empty());
  assert(!expr_match((w * 2) + 17, fx + 17, matches) && matches.empty());
  assert(!expr_match(w * 3, 3 * x, matches) && matches.empty());
...
bool expr_match(Expr pattern, Expr expr, vector<Expr>& matches) {
  matches.clear();
  if (!pattern.defined() && !pattern.defined())
    return true;
  if (!pattern.defined() || !pattern.defined())
    return false;
