...
#include "jmlang/Optimizer/Simplify.h"
...
bool is_const_power_of_two(Expr e, int* bits) {
  ...
  if (b)
    return is_const_power_of_two(b->value, bits);

  const Cast* c = e.as<Cast>();
  if (c)
    return is_const_power_of_two(c->value, bits);
...
void check_representable(Type t, int x) {
  ...
  if (result != x) {
    std::cerr << "Integer constant " << x << " would be converted to " << result
...
Expr jmlang_log(Expr x_full) {
  ...
  Expr nan =
      Call::make(Float(32), "nan_f32", std::vector<Expr>(), Call::Extern);
  Expr neg_inf =
      Call::make(Float(32), "neg_inf_f32", std::vector<Expr>(), Call::Extern);
...
Expr jmlang_exp(Expr x_full) {
  ...
  Expr inf =
      Call::make(Float(32), "inf_f32", std::vector<Expr>(), Call::Extern);
